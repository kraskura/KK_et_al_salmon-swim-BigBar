...
mod.size.ucritS<-lm(SWIM_cms~ LENGTH_cm + Species_latin, data.cm.ucrit, na.action = na.exclude)
...
BIC(mod.size.ucritS, mod.size.ucrit)
...
# *********** START ***************************************
# mixed models, species random intercept
# ******************************************************
data.cmTNorm.use.ucrit<-data.cmTNorm.use[data.cmTNorm.use$Test_performance2 == "Ucrit",]

# proportional (standardised swim performance)
mod.size.ucrit<-lmer(SWIM_cms.prop0~ Temp_test_mean + Temp_test_mean^2 + (1|Species_latin), data.cmTNorm.use.ucrit)

# all test data as recorded:
mod.size.ucritI<-lmer(SWIM_cms~ Temp_test_mean + Temp_test_mean^2 + (1|Species_latin), data.cmT.use)
...
# Field all swim speeds
# ******************************************************
data.cmTNorm.use.ucrit$Species_latin<-factor(data.cmTNorm.use.ucrit$Species_latin)
data.cmTNorm.use.ucrit$Species_latin<-factor(data.cmTNorm.use.ucrit$Species_latin)

# proportional (standardised swim performance) <<<<<<<<<<<<<<<
# species specific fits, CI of models, resid plots
# 1. pink : "Oncorhynchus gorbuscha"
mod.pink<-lm(SWIM_cms.prop0~ Temp_test_mean + Temp_test_mean^2, na.action=na.omit, data = data.cmTNorm.use.ucrit[(data.cmTNorm.use.ucrit$Species_latin == "Oncorhynchus gorbuscha"),])
...
# 2. sockeye: Oncorhynchus nerka
mod.soc<-lm(SWIM_cms.prop0~ Temp_test_mean + Temp_test_mean^2, na.action=na.omit, data = data.cmTNorm.use.ucrit[(data.cmTNorm.use.ucrit$Species_latin == "Oncorhynchus nerka"),])
...
# 3. trouts: Oncorhynchus mykiss
mod.trouts<-lm(SWIM_cms.prop0~ Temp_test_mean + Temp_test_mean^2, na.action=na.omit, data = data.cmTNorm.use.ucrit[(data.cmTNorm.use.ucrit$Species_latin == "Oncorhynchus mykiss"),])
...
# 4. chinook: Oncorhynchus tshawytscha
mod.chin<-lm(SWIM_cms.prop0~ Temp_test_mean + Temp_test_mean^2, na.action=na.omit, data = data.cmTNorm.use.ucrit[(data.cmTNorm.use.ucrit$Species_latin == "Oncorhynchus tshawytscha"),])
...
mod.coho<-lm(SWIM_cms.prop0~ Temp_test_mean + Temp_test_mean^2, na.action=na.omit, data = data.cmTNorm.use.ucrit[(data.cmTNorm.use.ucrit$Species_latin == "Oncorhynchus kisutch"),])
...
mod.salar<-lm(SWIM_cms.prop0~ Temp_test_mean + Temp_test_mean^2, na.action=na.omit, data = data.cmTNorm.use.ucrit[(data.cmTNorm.use.ucrit$Species_latin == "Salmo salar"),])
...
data.cmT.use$Temp_test_meanQUAD <- data.cmT.use$Temp_test_mean^2
### ****** ALL DATA AS RECORDED, not normalized:
# species specific fits, CI of models, resid plots
# 1. pink : "Oncorhynchus gorbuscha"
modI.pink<-lm(SWIM_cms~ poly(Temp_test_mean,2, raw = T), na.action=na.exclude, data = data.cmT.use[(data.cmT.use$Species_latin == "Oncorhynchus gorbuscha"),])
# modI.pink2<-lm(SWIM_cms~ Temp_test_mean + Temp_test_meanQUAD, na.action=na.exclude, data = data.cmT.use[(data.cmT.use$Species_latin == "Oncorhynchus gorbuscha"),]) #<- produces the same results
...
nrow(data.cmT.use[(data.cmT.use$Species_latin == "Oncorhynchus gorbuscha" & data.cmT.use$Test_performance2 == "Umax"),])
nrow(data.cmT.use[(data.cmT.use$Species_latin == "Oncorhynchus gorbuscha" & data.cmT.use$Test_performance2 == "Ucrit"),])

nrow(data.cmT.use[(data.cmT.use$Species_latin == "Oncorhynchus gorbuscha" ),])

# 2. sockeye: Oncorhynchus nerka
modI.soc<-lm(SWIM_cms~ poly(Temp_test_mean,2, raw = T), na.action=na.exclude, data = data.cmT.use[(data.cmT.use$Species_latin == "Oncorhynchus nerka"),])
...
# 3. trouts: Oncorhynchus mykiss
modI.trouts<-lm(SWIM_cms~ poly(Temp_test_mean,2, raw = T), na.action=na.exclude, data = data.cmT.use[(data.cmT.use$Species_latin == "Oncorhynchus mykiss"),])
...
# 4. chinook: Oncorhynchus tshawytscha
modI.chin<-lm(SWIM_cms~ poly(Temp_test_mean,2, raw = T), na.action=na.exclude, data = data.cmT.use[(data.cmT.use$Species_latin == "Oncorhynchus tshawytscha"),])
...
# 5. coho: Oncorhynchus kisutch
modI.coho<-lm(SWIM_cms~ poly(Temp_test_mean,2, raw = T), na.action=na.exclude, data = data.cmT.use[(data.cmT.use$Species_latin == "Oncorhynchus kisutch"),])
...
# 6. Atlantic: Salmo salar
modI.salar<-lm(SWIM_cms~ poly(Temp_test_mean,2, raw = T), na.action=na.exclude, data = data.cmT.use[(data.cmT.use$Species_latin == "Salmo salar"),])
...
# 7 Chum 
modI.chum<-lm(SWIM_cms~ poly(Temp_test_mean,2, raw = T), na.action=na.exclude, data = data.cmT.use[(data.cmT.use$Species_latin == "Oncorhynchus keta"),])
...
# summary of all fits
# proportional standardised temp data 
...
# find the peaks for species specific poly fits 
...
o.salar<- optimize(f = P.salar, interval = c(3,27), maximum = TRUE)
...
p1.cmF<-ggMarginal(p1.cmF, groupColour = TRUE, groupFill = TRUE,  type = "histogram", alpha = 1,
           yparams = list(),  xparams = list(binwidth = 3))
...
p2.PREDFIT_full <- ggplot(data= dd_TempI, aes(y=pred.mod, x=Temp_test_mean, fill=Species_latin,
  ...
  geom_line(aes(y=pred.mod, x= Temp_test_mean), size=2)+ 
  geom_text(mapping = aes(x = opt.T, y = 0, colour = Species_latin, fill = NULL, shape = NULL), angle = 90, label = round(fit.peaks$opt.T,1), size=3,  fontface = "bold",  data = fit.peaks, alpha = 1)+
...
p2.PREDFIT_full<-p2.PREDFIT_full+theme(legend.position = "none", plot.margin=margin(t=-3,unit="cm"))
p2.PREDFIT_full


p2.temp_hist<-ggplot(data, aes(x = Temp_test_mean, y = Species_latin, fill = Species_latin, color = Species_latin)) +
...
ggsave(filename = "/Users/kristakraskura/Desktop/BOX/UCSB/Research/Pacific Salmon/Manuscr Swimming Lit Rev /Figures/FigTEMP_FULL.png",
       plot = p2.PREDFIT_full, width = 4.2, height = 4,units = "in")


cowplot::plot_grid( p2.temp_hist,p2.PREDFIT_full,
                   nrow = 2, ncol =1, align = "hv", rel_heights = c(0.5, 1)) %>%
  ggsave(filename = "/Users/kristakraskura/Desktop/BOX/Publications - Work/Kraskura et al CJFAS 2022 salmon swim/Figures/FigTemp_hist_TPC_update.png",
...
cowplot::plot_grid(p1.cm, p1.cmF,
                   ...
                   label_x = c(0.1, 0.1),
                   label_y = c(0.9)) %>%
  ggsave(filename = "/Users/kristakraskura/Desktop/BOX/Publications - Work/Kraskura et al CJFAS 2022 salmon swim/Figures/FigSIZE_update.png",
