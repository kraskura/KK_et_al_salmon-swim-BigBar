...
# recovery ref table - for prospectus and thesis >>>> ---------

recov_table_sum<-as.data.frame(data %>% 
  group_by(Reference_number_1) %>% 
  dplyr:::summarize(species = paste(unique(Species_latin), collapse = ', '),
                    test = paste(unique(Test_performance2), collapse = ', ')))

write.csv(file="/Users/kristakraskura/Desktop/BOX/UCSB/Research/Salmon/Big Bar Rockslide /Recovery_Summary_june24.csv", recov_table_sum, row.names=FALSE)
...
# summary by different test performances
data_sum374<-data %>% 
...
# save for PROSPECTUS
write.csv(file="/Users/kristakraskura/Desktop/BOX/UCSB/PhD-docs-milestones/Prospectus/Supporting Data/KK_ch4_summarySpeciesCM.csv", data_sum5, row.names=FALSE)
write.csv(file="/Users/kristakraskura/Desktop/BOX/UCSB/PhD-docs-milestones/Prospectus/Supporting Data/KK_ch4_summarySpeciesBL.csv", data_sum5dpBL, row.names=FALSE)
...
# ***
data_sum6<-as.data.frame(data[c(!is.na(data$swim_speed_MEAN_cm_s) & !data$Recovery=="n"),] %>% 
...
data_sum6dp<-as.data.frame(data[!is.na(data$swim_speed_MEAN_cm_s),] %>% 
  group_by(Recovery) %>% 
   dplyr:::summarize(n_studies_ref_dp=paste(length(unique(Reference_number_1)), " (", length(swim_speed_MEAN_cm_s),") ", sep = ''),
                    studies = paste(unique(Reference_number_1), collapse = ', ')))
#datapoints BL
data_sum6dpBL<-as.data.frame(data[!is.na(data$swim_speed_MEAN_BL_s),]  %>% 
  group_by(Recovery) %>% 
   dplyr:::summarize(n_studies_ref_dp=paste(length(unique(Reference_number_1)), " (", length(swim_speed_MEAN_cm_s),") ", sep = ''),
                    studies = paste(unique(Reference_number_1), collapse = ', ')))
...
# Formatting categories for prospectus
data$species_color_size<-NA
data$species_color_size[data$Species_latin=="Oncorhynchus tshawytscha" | data$Species_latin=="Oncorhynchus keta"]<-"#db3056"
data$species_color_size[data$Species_latin=="Oncorhynchus gorbuscha"]<-"#ffcc00"
data$Fish_Conditions_size<-0.8
data$Fish_Conditions_size[data$Fish_Conditions=="fallback"]<-1.5
data$field_lab_stroke<-NA
data$field_lab_stroke<-0.8
data$field_lab_stroke[data$Test_performance2=="Field"]<-1.5
cols<-c("white", "#ff9d76","grey", "#ef4339", "#086972")
...
fish_cond_prospectus<-ggplot(data = data[c(!is.na(data$Fish_Conditions) & !(data$Species_latin=="Salmo salar") & !(data$Species_latin=="Oncorhynchus mykiss")),], aes(y=swim_speed_MEAN_cm_s, x=Species_latin, fill=Species_latin))+
  geom_boxplot( alpha=0.8, outlier.shape=NA, show.legend = FALSE) +
  geom_point(aes(shape=factor(Fish_Conditions), color=factor(Fish_Conditions),alpha=as.numeric(Fish_Conditions_size), stroke=Fish_Conditions_size), position=position_jitterdodge(),show.legend=FALSE, size=4)+
  scale_shape_manual(values = c(22,21,21,24,21,21,21,21))+
  scale_alpha_continuous(range=c(0.4,1))+
  scale_color_manual(values=c(cols[4],"black","black","black","black","black","black","black"))+
  scale_fill_manual(values=c(cols[3], cols[2], cols[1], cols[1], cols[2]))+
  scale_x_discrete(breaks = c("Oncorhynchus spp.", "Oncorhynchus gorbuscha", "Oncorhynchus keta", "Oncorhynchus kisutch", "Oncorhynchus nerka", "Oncorhynchus tshawytscha", "Oncorhynchus masou"),  labels=c("Oncorhynchus spp.", "O. gorbuscha", "O. keta", "O. kisutch", "O. nerka", "O. tshawytscha","O. masou")) 
ggformat(fish_cond_prospectus, title = "", y_title = "Swim speed (cm/s)", x_title = "")
fish_cond_prospectus<-fish_cond_prospectus + theme( legend.title = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))

fish_cond_prospectusBL<-ggplot(data = data[c(!is.na(data$Fish_Conditions) & !(data$Species_latin=="Salmo salar") & !(data$Species_latin=="Oncorhynchus mykiss")),], aes(y=swim_speed_MEAN_BL_s, x=Species_latin, fill=Species_latin))+
  geom_boxplot(alpha=0.8, outlier.shape=NA, show.legend = FALSE) +
 geom_point(aes(shape=factor(Fish_Conditions), color=factor(Fish_Conditions),alpha=as.numeric(Fish_Conditions_size), stroke=Fish_Conditions_size), position=position_jitterdodge(),show.legend=FALSE, size=4)+
  scale_shape_manual(values = c(22,21,21,24,21,21,21,21))+
  scale_alpha_continuous(range=c(0.4,1))+
  ...
  scale_fill_manual(values=c(cols[3], cols[2], cols[1], cols[1], cols[2]))+
  scale_x_discrete(breaks = c("Oncorhynchus spp.", "Oncorhynchus gorbuscha", "Oncorhynchus keta", "Oncorhynchus kisutch", "Oncorhynchus nerka", "Oncorhynchus tshawytscha", "Oncorhynchus masou"),  labels=c("Oncorhynchus spp.", "O. gorbuscha", "O. keta", "O. kisutch", "O. nerka", "O. tshawytscha","O. masou")) 
ggformat(fish_cond_prospectusBL, title = "", y_title = "Relative swim speed (BL/s)", x_title = "Species")
fish_cond_prospectusBL<-fish_cond_prospectusBL + theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title = element_blank())

# Figure 1 Prospectus >>>>> -----------
# Fig1_prospectus<- plot_grid(fish_cond_prospectus, fish_cond_prospectusBL,
#                          align = "h",
#                          axis = "l",
#                          nrow = 2,
#                          ncol = 1,
...
#                     ) %>% 
# # fig_part4.1
# ggsave(filename = "/Users/kristakraskura/Desktop/BOX/UCSB/PhD-docs-milestones/Prospectus/Figures and ppt/Chapter_Figure2.png", width = 7, height = 14)
#  
...
swim_cond<-ggplot(data = data[!is.na(data$Swim_Conditions2),], aes(y=swim_speed_MEAN_cm_s, x=Species_latin, fill=Swim_Conditions2))+
  geom_boxplot(fill = "white", alpha=0.8, outlier.shape=NA) +
  geom_point(positsion=position_jitterdodge(), alpha=0.7, pch=21, size=2)+
...
swim_test<-ggplot(data = data[c(!is.na(data$Test_performance) & !(data$Species_latin=="Salmo salar") & !(data$Species_latin=="Oncorhynchus mykiss") & !(data$Species_latin=="Oncorhynchus masou")),], aes(y=swim_speed_MEAN_cm_s, x=Species_latin, fill=Species_latin))+
  geom_boxplot(alpha=0.8, outlier.shape=NA, show.legend = FALSE) + # shape=Indiv_group)
  geom_point(aes(shape=factor(Test_performance2), color=factor(Test_performance2),alpha=as.numeric(field_lab_stroke), stroke=field_lab_stroke), position=position_jitterdodge(), size=4, show.legend = FALSE)+
  scale_shape_manual(values = c(21,22,21,21,21,21,21,21,21))+
  scale_alpha_continuous(range=c(0.4,1))+
  scale_color_manual(values=c("black",cols[5],"black","black","black","black","black", "black"))+
  scale_fill_manual(values=c(cols[3], cols[2], cols[1], cols[1], cols[1],cols[2]))+
  scale_x_discrete(breaks = c("Oncorhynchus spp.", "Oncorhynchus gorbuscha", "Oncorhynchus keta", "Oncorhynchus kisutch", "Oncorhynchus nerka", "Oncorhynchus tshawytscha"),  labels=c("Oncorhynchus spp.", "O. gorbuscha", "O. keta", "O. kisutch", "O. nerka", "O. tshawytscha")) 
ggformat(swim_test, title = "", y_title = "Swim speed (cm/s)", x_title = "Species")
swim_test<-swim_test + theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none")


swim_testBL<-ggplot(data = data[c(!is.na(data$Test_performance) & !(data$Species_latin=="Salmo salar") & !(data$Species_latin=="Oncorhynchus mykiss") & !(data$Species_latin=="Oncorhynchus masou")),], aes(y=swim_speed_MEAN_BL_s, x=Species_latin, fill=Species_latin))+
  geom_boxplot(alpha=0.8, outlier.shape=NA, show.legend = FALSE) + # shape=Indiv_group)
  geom_point(aes(shape=factor(Test_performance2), color=factor(Test_performance2),alpha=as.numeric(field_lab_stroke), stroke=field_lab_stroke), position=position_jitterdodge(), size=4, show.legend = FALSE)+
  scale_shape_manual(values = c(21,22,21,21,21,21,21,21,21))+
  scale_alpha_continuous(range=c(0.4,1))+
  ...
  scale_fill_manual(values=c(cols[3], cols[2], cols[1], cols[1], cols[1],cols[2]))+
  scale_x_discrete(breaks = c("Oncorhynchus spp.", "Oncorhynchus gorbuscha", "Oncorhynchus keta", "Oncorhynchus kisutch", "Oncorhynchus nerka", "Oncorhynchus tshawytscha"),  labels=c("Oncorhynchus spp.", "O. gorbuscha", "O. keta", "O. kisutch", "O. nerka", "O. tshawytscha")) 
ggformat(swim_testBL, title = "", y_title = "Relative swim speed (BL/s)", x_title = "Species")
swim_testBL<-swim_testBL + theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none")


Fig1ABCD_prospectus<- plot_grid(fish_cond_prospectus, fish_cond_prospectusBL,
                            swim_test, swim_testBL,
                         align = "h",
                         axis = "l",
                         nrow = 2,
                         ncol = 2,
                         labels = "AUTO",
                         label_x = c(0.14, 0.14, 0.14, 0.14),
                         label_y = c(0.92, 0.92, 0.92, 0.92),
                         label_size = 23
                    ) %>% 
# fig_part4.1
ggsave(filename = "/Users/kristakraskura/Desktop/BOX/UCSB/PhD-docs-milestones/Prospectus/Figures and ppt/Chapter3_Figure2ABCD.png", width = 14, height = 14)
...
big_fish<-data[c((data$Species_latin=="Oncorhynchus tshawytscha") | (data$Species_latin=="Oncorhynchus keta")),]

temp1<-ggplot(data=data[c(!(data$Species_latin=="Salmo salar") | !(data$Species_latin=="Oncorhynchus mykiss") | !(data$Species_latin=="Oncorhynchus masou")),], aes(Temp_test_mean, swim_speed_MEAN_cm_s))+
  scale_fill_manual(values=c(cols[3], cols[2], cols[1], cols[1], cols[1],cols[2], cols[1]))+
  geom_point( alpha=0.5, colour="black", pch=21, size=2)+
  geom_point( data=big_fish, alpha=1, fill=cols[2], pch=21, size=4)+
  ylim(0,700)
  # geom_abline(intercept = coef(lm_big_fish)[1], slope=coef(lm_big_fish)[2], colour=cols[4], lty=2)
 ggformat(temp1, print=TRUE, y_title = "Swim speed (cm/s)", x_title = "Temperature (deg. C)", title ="")
temp1<-temp1 +theme (legend.title = element_blank())


temp1BL<-ggplot(data=data[c(!(data$Species_latin=="Salmo salar") | !(data$Species_latin=="Oncorhynchus mykiss") | !(data$Species_latin=="Oncorhynchus masou")),], aes(Temp_test_mean, swim_speed_MEAN_BL_s))+
  scale_fill_manual(values=c(cols[3], cols[2], cols[1], cols[1], cols[1],cols[2], cols[1]))+
  geom_point( alpha=0.5, colour="black", pch=21, size=2)+
  geom_point( data=big_fish, alpha=1, fill=cols[2], pch=21, size=4)
  # ylim(0,700)
  # geom_abline(intercept = coef(lm_big_fish)[1], slope=coef(lm_big_fish)[2], colour=cols[4], lty=2)
 ggformat(temp1BL, print=TRUE, y_title = "Relative swim speed (BL/s)", x_title = "Temperature (deg. C)", title ="")
temp1BL<-temp1BL +theme (legend.title = element_blank())

Fig1ABCDEF_prospectus<- plot_grid(fish_cond_prospectus, fish_cond_prospectusBL,
                            swim_test, swim_testBL,
                            temp1, temp1BL,
                         align = "v",
                         axis = "l",
                         ...
                         ncol = 2,
                         labels = "AUTO",
                         label_x = c(0.14, 0.14, 0.14, 0.14, 0.14, 0.14),
                         label_y = c(0.92, 0.92, 0.92, 0.92, 0.92, 0.92),
                         label_size = 23, rel_heights = c(1.15,1.15,1)
                    ) %>% 
# fig_part4.1
ggsave(filename = "/Users/kristakraskura/Desktop/BOX/UCSB/PhD-docs-milestones/Prospectus/Figures and ppt/Chapter3_Figure2ABCDEF.png", width = 14, height = 20)
...
# Figure 4. Prospectus >>>>> ------
data=data[c(!(data$Species_latin=="Salmo salar") | !(data$Species_latin=="Oncorhynchus mykiss") | !(data$Species_latin=="Oncorhynchus masou")),]

hist5 <- gghistogram(data = data[c(!is.na(data$Length_MEAN_cm)),], x = "Length_MEAN_cm",
   bins = 40,
   add = "mean",
   rug = TRUE,
   alpha=0.7,
   ...
   fill = "Species_latin",
   # title= "All studies",
   palette = c("grey", cols[2],  "black", "black", cols[2]),
   ylab="Count",
   legend="none",
   legend.title = "",
   add.density=TRUE,
   facet.by = c("Species_latin", rows=1),
   xlab="Fish size (cm)") 
hist5

Fig2Hist_prospectus<- plot_grid(hist5,
                         align = "v",
                         axis = "l",
                         nrow = 1,
                         ncol = 1
...
# fig_part4.1
ggsave(filename = "/Users/kristakraskura/Desktop/BOX/UCSB/PhD-docs-milestones/Prospectus/Figures and ppt/Chapter3_Figure2hist.png", width = 4, height = 10)
